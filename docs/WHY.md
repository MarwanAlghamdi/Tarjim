...
Every section here records something that was measured, not assumed. The
[README](../README.md) tells you what to do; this file tells you why, and what
happens if you do it differently.

---

## Setup details the README skips

### Vendoring the PDF.js viewer
...
### What each option field accepts
...
## Why Ollama answers 403 until you run a script
...
reuses it afterwards. **Keep it.** The extension ID is a hash of it, so signing a
new release with a different key makes the browser treat it as a different
extension and the user loses every saved setting instead of upgrading.

`npm run package` prints the ID it derived; keep that key file out of the
repository and off any public host.
...
OLLAMA_ENDPOINT=192.168.1.50:11434 OLLAMA_MODEL=some-other-model npm run verify:live
```
...
The README screenshots are regenerated with:

```bash
node tools/screenshots.mjs      # needs real Ollama and the model pulled
node tools/check-readme.mjs all # link, command, shape and preservation lint
```

`screenshots.mjs` drives the real extension against real Ollama on purpose: a
screenshot taken against the test stub would show `ترجمة تجريبية`, which is a
picture of the harness rather than of the product. `check-readme.mjs` has a
`selftest` mode that runs every check against a deliberately broken fixture
tree and fails if any of them reports it clean.
...
tools/                 package.sh, setup-ollama-cors.sh, vendor-pdfjs.sh,
                       patch-pdfjs.py, verify-brave.mjs, screenshots.mjs,
                       check-readme.mjs
docs/                  WHY.md and the README screenshots
